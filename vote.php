<?php
    ...
    $req_like = $bdd->prepare("SELECT id FROM v_like WHERE id_account=?");
    $req_like->execute(array($_SESSION['id']));
    $like_exist = $req_like->rowCount();

    $req_dislike = $bdd->prepare("SELECT id FROM v_dislike WHERE id_account=?");
    $req_dislike->execute(array($_SESSION['id']));
    ...
    $id_actor = (int) htmlspecialchars($_GET['id_actor']);
        if ($like_exist == 0 AND isset($_SESSION['id']) AND isset($id_actor) AND isset($_POST['vote_like'])) 
            {//Si l'utilisateur connecté n'a pas liké, on vérifie pour dislike
                ...
                if ($dislike_exist == 0) {//S'il n'y a pas de vote de l'utilisateur on enregistre le like
